...
function Logger({
  ...
  function print(level, event, payload) {
    ...
    if (window.LOG_LEVEL && _config.LOG_LEVEL_PRIORITY.indexOf(window.LOG_LEVEL) !== -1) {
      consoleLogLevel = window.LOG_LEVEL;
    }

    if (_config.LOG_LEVEL_PRIORITY.indexOf(level) > _config.LOG_LEVEL_PRIORITY.indexOf(consoleLogLevel)) {
      return;
    }
  ...
  function immediateFlush() {
    if (!(0, _src2.isBrowser)() || !(0, _src2.isElectron)() && window.location.protocol === _constants.PROTOCOL.FILE || !events.length && !tracking.length) {
